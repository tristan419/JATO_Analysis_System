...
def upgrade() -> None:
    bind = op.get_bind()
    if not inspect(bind).has_table("role_upgrade_requests", schema="auth"):
        op.create_table(
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_role_upgrade_requests_status "
        "ON auth.role_upgrade_requests (status)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_role_upgrade_requests_user "
        "ON auth.role_upgrade_requests (user_id)"
    )


def downgrade() -> None:
    op.execute("DROP TABLE IF EXISTS auth.role_upgrade_requests")
