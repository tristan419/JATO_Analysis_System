...
def probe_pipeline_failures() -> dict[str, Any]:
    ...
    Reads standard pipeline_status/*.json records first, with legacy
    scheduled_fetch_status.json/report fallbacks handled by the status service.
    """
    from app.services import hermes_pipeline_status_service as pipeline_status_service

    findings: list[dict[str, Any]] = []
    pipeline_status_service._project_root = _root()
    status_records = pipeline_status_service.list_pipeline_statuses(include_missing=True)
    for record in status_records:
        health = pipeline_status_service.classify_pipeline_health(record)
        ...
        findings.append({
            ...
            "failedCount": record.get("failedCount", 0),
            "warningCount": record.get("warningCount", 0),
            "artifactRefs": record.get("artifactRefs", []),
            "statusRecord": record,
            "recommendedAction": _pipeline_recommended_action(record),
            **({"ageHours": health["ageHours"]} if "ageHours" in health else {}),
        })
...
def _pipeline_recommended_action(record: dict[str, Any]) -> str:
    """Return a debugging action for a pipeline status record."""
    pipeline = str(record.get("pipelineId") or "")
    if record.get("status") == "missing":
        return f"Ensure {pipeline} writes hermes/reports/pipeline_status/{pipeline}.json."
    if pipeline == "source_quality":
        return "Check hermes-source-quality.timer and source_quality_report.json."
    return f"Inspect journalctl -u {_pipeline_unit_name(pipeline)} and linked artifacts."


def _pipeline_unit_name(pipeline: str) -> str:
    """Map pipeline id to systemd unit name for debugging."""
    mapping = {
        "country_news_sync": "jato-country-news-sync",
        "country_news_sync_b": "jato-country-news-sync-b",
