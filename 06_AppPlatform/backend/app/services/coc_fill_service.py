...
def _record_from_pdf_row(
    ...
    *,
    ...
    return WvtaCocRecord(
        ...
        version=at(-1),
        sales_name=at(7),
        valid_from=_parse_date(at(8), day_first=True),
        valid_to=_parse_date(at(9), day_first=True),
        comments=at(10),
                ...
                for table in tables:
                    for row_index, row in enumerate(table, start=1):
                        ...
                        for cell_index, cell in enumerate(cells):
                            material = _normalize_material_group(cell)
                            if not material:
                                continue
...
def fill_coc_workbook(
    *,
    ...
    workbook, targets_by_sheet, material_rows = _extract_material_rows(
        workbook_path,
    ...
    if not material_rows:
        if sheet_names:
            ...
            raise HTTPException(
                status_code=422,
                detail=(
                    f"指定 Sheet 未识别到物料号组列或包含 ** 的物料号组：{_display_names(requested)}。"
...
def _override_key(payload: dict[str, Any]) -> tuple[str, int, str]:
    return (
        ...
        _normalize_material_group(payload.get("materialGroup") or payload.get("material_group")),
    )
    ...
    *,
    ...
    for result_row in range(2, (result_sheet.max_row or 0) + 1):
        ...
        try:
            ...
            source_row = 0
        source_material = _normalize_material_group(result_sheet.cell(result_row, 4).value)
    ...
    try:
        for override, _decision, candidate, _group, mode in prepared:
            ...
            previous_values[(sheet_name, row_number, material_group)] = (
                _cell_text(sheet.cell(row_number, targets.wvta_col).value),
                _cell_text(sheet.cell(row_number, targets.coc_col).value),
            )
