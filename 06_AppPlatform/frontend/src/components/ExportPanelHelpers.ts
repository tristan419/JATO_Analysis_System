...
export const TICK_FORMATS: { v: string; l: string }[] = [
  { v: "", l: "保留原始" }, { v: "d", l: "整数" }, { v: ",.0f", l: "千分位整数" },
  { v: ",.1f", l: "千分位1位" }, { v: ".0%", l: "百分比(0-1)" }, { v: ".2s", l: "科学计数" },
];
export const DEFAULT_LABEL_MODES: ExportLabelMode[] = ["off","value","series","model","sales","model+value","model+sales","percent"];
export const LABEL_MODE_LABELS: Record<ExportLabelMode, string> = {
  off: "关闭",
  value: "value",
  series: "series",
  model: "model",
...
export const LABEL_POSITIONS = ["auto","inside","outside","top","top center","middle","bottom center"];
export const LABEL_OVERLAP_STRATEGIES: Array<{ value: ExportLabelOverlapStrategy; label: string }> = [
  { value: "all", label: "All" },
...
export function normalizeExportLabelStrategy(strategy: ExportLabelOverlapStrategy | undefined): ExportLabelOverlapStrategy {
  if (strategy === "smart") return "smart_top";
  if (strategy === "none" || !strategy) return "all";
  return strategy;
}

export function resolveLabelFontSize(settings: ExportSettings): number {
...
export function getSeriesDefaultColor(name: string, fallback: string): string {
...
export function collectExportSeriesNamesFromGraphDiv(graphDiv?: HTMLElement | null): string[] {
