...
const HERO_PAGE_ITEMS: Array<{ key: HeroProductPageKey; code: string; label: string; sublabel: string }> = [
  ...
  { key: "heroTrend", code: "05", label: "China Trend", sublabel: "中国车趋势" },
  { key: "heroDistribution", code: "06", label: "China Markets", sublabel: "中国车分布" },
];
...
const FALLBACK_HERO_PRODUCT_FUEL_OPTIONS: MarketScanCountryOption[] = [
  ...
  { value: "HEV", label: "HEV" },
  { value: "MHEV", label: "MHEV" },
  { value: "REEV", label: "REEV" },
  { value: "ICE", label: "ICE" },
];
const HERO_PRODUCT_TOP_N_PRESETS = [10, 20, 30] as const;
const HERO_PRODUCT_MIN_TOP_N = 10;
const HERO_PRODUCT_MAX_TOP_N = 30;
...
function rankingTrendSourceTableForSalesMode(mode: HeroProductSalesMode): string {
  return mode === "month" ? "monthly_brand_ranking" : "ytd_brand_ranking";
}
...
function normalizeHeroFuelTypes(values: Iterable<string>): string[] {
  ...
  for (const value of values) {
    const normalized = normalizeHeroFuelType(value);
    if (!normalized || seen.has(normalized)) continue;
    selected.push(normalized);
    seen.add(normalized);
  }
  return selected.length > 0 ? selected : [...DEFAULT_HERO_PRODUCT_FUEL_TYPES];
}

function parseFuelTypesParam(value: string | null): string[] {
  if (!value) return [...DEFAULT_HERO_PRODUCT_FUEL_TYPES];
  return normalizeHeroFuelTypes(value.split(","));
}

function sameFuelTypes(left: string[], right: string[]): boolean {
...
function parseTopNParam(value: string | null): number {
  ...
  if (!Number.isFinite(numeric)) return HERO_PRODUCT_MIN_TOP_N;
  return Math.max(HERO_PRODUCT_MIN_TOP_N, Math.min(HERO_PRODUCT_MAX_TOP_N, Math.round(numeric)));
}
...
function resolveCountryOption(options: MarketScanCountryOption[], value: string, fallback: MarketScanCountryOption): MarketScanCountryOption {
  const normalized = normalizeCountryText(value);
  if (!normalized) return fallback;
  return options.find((option) => (
    normalizeCountryText(option.value) === normalized ||
    normalizeCountryText(option.label) === normalized
  )) ?? fallback;
}

function countryRankingMatches(ranking: HeroProductCountryRanking, country: MarketScanCountryOption): boolean {
  const values = [
    ranking.country.value,
    ranking.country.label,
  ].map(normalizeCountryText);
  return values.includes(normalizeCountryText(country.value)) || values.includes(normalizeCountryText(country.label));
}

function countryRankingForTrendPage(page: HeroProductDeckResponse["pages"]["topTrend"], country: MarketScanCountryOption): HeroProductCountryRanking {
  if (countryRankingMatches(page.countryRanking, country)) return page.countryRanking;
  return Object.values(page.countryRankings ?? {}).find((ranking) => countryRankingMatches(ranking, country)) ?? page.countryRanking;
}

function hasCountryRankingForTrendPage(page: HeroProductDeckResponse["pages"]["topTrend"], country: MarketScanCountryOption): boolean {
  return countryRankingMatches(countryRankingForTrendPage(page, country), country);
}
...
function buildBenchmarkInsight(rows: HeroProductModelRow[], productRows: HeroProductModelRow[], priceSource: HeroProductPriceSource, showChannel: boolean): HeroProductInsight {
  ...
  const benchmarkHeadline = missingSpecs.length > 0
    ? `${batteryLeader?.model ?? salesLeader?.model ?? "中国车型"}已有电池/销量基准，${missingSpecs.join("、")}仍待补齐；先用现有规格表定位对标缺口。`
  ...
  if (showChannel) {
    return {
      ...
      summary: `左侧保留 ${topLabel} 销量排序和渠道结构，右侧用中国车型规格表复核续航、性能、充电和 ${priceSource.toUpperCase()} 价格，判断渠道放量是否有产品力支撑。`,
      cards: [
  ...
  return {
    ...
    summary: `左侧复刻 ${topLabel} 销量基准，右侧中国车型下沉到续航、电池、电耗、零百、充电和 ${priceSource.toUpperCase()} 价格，先看结论再看单项差距。`,
    cards: [
      rangeOrBatteryCard,
...
function buildTrendInsight(page: HeroProductDeckResponse["pages"]["topTrend"], priceSource: HeroProductPriceSource, variant: "top" | "hero"): HeroProductInsight {
  ...
  const headline = variant === "hero"
    ? `${seriesLabel(risingSeries)}中国车型拉升最明显，${seriesLabel(fallingSeries)}承压；价格变化需要和销量曲线同步解释。`
...
function buildDistributionInsight(page: HeroProductDeckResponse["pages"]["topDistribution"], variant: "top" | "hero"): HeroProductInsight {
  ...
  const headline = variant === "hero"
    ? `${topCountry?.country ?? "核心市场"}是中国车型放量核心，${salesLeader?.model ?? "主力车型"}总量最高；同级判断要看国家覆盖和驱动结构。`
...
export function HeroProductAnalysisView({ onSwitchToTransfer }: { onSwitchToTransfer: () => void }) {
  ...
  const [selectedFuelTypes, setSelectedFuelTypes] = useState<string[]>(() => parseFuelTypesParam(searchParams.get("fuelTypes")));
  const [topN, setTopN] = useState(() => parseTopNParam(searchParams.get("topN")));
  ...
  const [distributionLayout, setDistributionLayout] = useState<HeroProductDistributionLayout>(() => (
    isDistributionLayout(searchParams.get("distributionLayout")) ? searchParams.get("distributionLayout") as HeroProductDistributionLayout : "ranked"
  ));
  ...
  const [trendDrawer, setTrendDrawer] = useState<{
    ...
    model?: string;
    sourceTable: string;
  }>({ open: false, brand: "", sourceTable: "ytd_brand_ranking" });
  ...
  const deckRequestPayload = useMemo(() => ({
    ...
    segment: "SUV A0",
    fuel_type: selectedFuelTypes[0] ?? DEFAULT_HERO_PRODUCT_FUEL_TYPES[0],
  ...
  const availableFuelOptions = useMemo<MarketScanCountryOption[]>(() => {
    const byValue = new Map<string, MarketScanCountryOption>();
    const sourceOptions = deck?.metadata.availableFuelTypes?.length
      ? deck.metadata.availableFuelTypes
      : FALLBACK_HERO_PRODUCT_FUEL_OPTIONS;
    for (const option of sourceOptions) {
      const value = normalizeHeroFuelType(option.value);
      if (!value) continue;
      byValue.set(value, { value, label: option.label || value });
    }
    for (const value of selectedFuelTypes) {
      const normalized = normalizeHeroFuelType(value);
      if (normalized && !byValue.has(normalized)) byValue.set(normalized, { value: normalized, label: normalized });
    }
    return Array.from(byValue.values());
  }, [deck?.metadata.availableFuelTypes, selectedFuelTypes]);
  const requestedFuelLabel = selectedFuelTypes.join(" / ");
  const selectedFuelLabel = loading ? requestedFuelLabel : deck?.metadata.selectedFuelType ?? requestedFuelLabel;
  const trendCountry = resolvedTrackingCountry ?? deck?.metadata.selectedTrackingCountry ?? null;
  const trendCountryValue = trendCountry?.value || trendCountry?.label || currentTrackingCountry || currentPriceCountry;
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    api.heroProductDeck(deckRequestPayload)
      .then((response) => {
  ...
  useEffect(() => {
    ...
    if (activePage !== "topTrend" && activePage !== "heroTrend") return;
    const trendPage = activePage === "heroTrend" ? deck.pages.heroTrend : deck.pages.topTrend;
    const reloadKey = [
      activePage,
      resolvedTrackingCountry.value,
      resolvedTrackingCountry.label,
    ...
    if (hasCountryRankingForTrendPage(trendPage, resolvedTrackingCountry)) {
      if (fallbackRankingReloadKeyRef.current === reloadKey) fallbackRankingReloadKeyRef.current = "";
      return;
    }
  ...
  useEffect(() => {
    ...
    if (!sameFuelTypes(selectedFuelTypes, DEFAULT_HERO_PRODUCT_FUEL_TYPES)) params.set("fuelTypes", selectedFuelTypes.join(","));
    ...
    if (distributionLayout !== "ranked") params.set("distributionLayout", distributionLayout);
  ...
  function handleDistributionModelTrendOpen(brand: string, model: string): void {
    ...
    setTrendDrawer({
      ...
      brand: nextBrand,
      model: nextModel,
      sourceTable: rankingTrendSourceTableForSalesMode(salesMode),
    });
  }
  ...
  function toggleFuelType(value: string): void {
    ...
    setSelectedFuelTypes((current) => {
      const selected = normalizeHeroFuelTypes(current);
      if (selected.includes(normalized)) {
        const next = selected.filter((item) => item !== normalized);
        return next.length > 0 ? next : selected;
      }
      return normalizeHeroFuelTypes([...selected, normalized]);
    });
  }

  function isolateFuelType(value: string): void {
    const normalized = normalizeHeroFuelType(value);
    if (!normalized) return;
    setSelectedFuelTypes([normalized]);
  }

  function handleFuelTypeClick(value: string, event: ReactMouseEvent<HTMLButtonElement>): void {
    if (event.detail >= 2) {
      isolateFuelType(value);
      return;
    }
    toggleFuelType(value);
  }

  function updateTopN(value: number): void {
    setTopN(Math.max(HERO_PRODUCT_MIN_TOP_N, Math.min(HERO_PRODUCT_MAX_TOP_N, Math.round(value))));
  }
  ...
  async function handleExportSlide(): Promise<void> {
    ...
    try {
      ...
      link.download = [
        "hero-product",
        sanitizeFileNameSegment(selectedFuelLabel),
  ...
  const narrative = deck
    ...
    : `按 ${selectedFuelLabel} / SUV A0 生成 Hero Product 六页分析。`;
    ...
    <div className="market-scan-shell hero-product-shell">
      <div className="market-scan-main">
        <section className="header-card dashboard-hero market-scan-hero hero-product-hero">
          <div className="dashboard-hero-head">
            <div className="dashboard-hero-copy market-scan-hero-copy">
              ...
              <p>从 SUV A0 {selectedFuelLabel} 动总下钻到 Top 车型和中国车型，复刻六页产品、趋势、价格和市场分布分析。</p>
        ...
        <DeckFloatingDrawer
          ...
          footer={(
            <>
              <span className="market-scan-toolbar-chip">{selectedFuelLabel}</span>
        ...
        >
          <div className="deck-panel-grid">
            ...
            <div className="market-scan-field deck-panel-grid__wide">
              ...
              <small className="hero-product-control-hint">默认独立排序；国家对齐会统一 04/06 国家行轴，缺失国家保留空行。</small>
            </div>
            ...
            <div className="market-scan-fuel-bank deck-panel-grid__wide hero-product-fuel-bank">
              <span className="market-scan-fuel-bank-label">Powertrain</span>
              <div className="market-scan-fuel-chip-row">
                {availableFuelOptions.map((option) => {
                  const value = normalizeHeroFuelType(option.value);
                  ...
                  return (
                    <button
                      key={value}
                      type="button"
                      className={`market-scan-fuel-chip${checked ? " is-active" : ""}`}
                      onClick={(event) => handleFuelTypeClick(value, event)}
                      title="单击多选，双击只看此动总"
                      style={{
                        borderColor: checked ? color : undefined,
                        background: checked ? `${color}16` : undefined,
                      }}
                    >
                      <span className="market-scan-fuel-dot" style={{ backgroundColor: color }} aria-hidden="true" />
                      {option.label}
                    </button>
                  );
                })}
              </div>
              <small className="hero-product-control-hint">单击多选，双击 isolate；至少保留 1 个动总，六页按组合口径重算。</small>
            </div>
            <div className="market-scan-field deck-panel-grid__wide">
              <span>Top Models</span>
              <div className="hero-product-top-n-control">
                <div className="btn-group">
                  {HERO_PRODUCT_TOP_N_PRESETS.map((preset) => (
                    <button key={preset} type="button" className={`btn btn-sm ${topN === preset ? "btn-primary" : "btn-ghost"}`} onClick={() => updateTopN(preset)}>
                      Top {preset}
                    </button>
                  ))}
                </div>
                <input
                  type="number"
                  min={HERO_PRODUCT_MIN_TOP_N}
                  max={HERO_PRODUCT_MAX_TOP_N}
                  step={1}
                  inputMode="numeric"
                  value={topN}
              ...
              <small className="hero-product-control-hint">TopN 联动 01/03/04/06；06 默认中国车池，Hero Models 有输入时覆盖 05/06。</small>
            </div>
            <label className="market-scan-field">
            ...
            <label className="market-scan-field deck-panel-grid__wide">
              ...
              <input value={topModelText} onChange={(event) => setTopModelText(event.target.value)} placeholder="可选，覆盖 04 全部 Top 车型" />
            </label>
            <label className="market-scan-field deck-panel-grid__wide">
              <span>Hero Models</span>
              <input value={heroModelText} onChange={(event) => setHeroModelText(event.target.value)} placeholder="可选，覆盖 05/06；空白则默认中国车 TopN" />
            </label>
                  ...
                  <div className="market-scan-slide-body hero-product-slide-body">
                    <div className="market-scan-slide-content hero-product-slide-content">
                      <HeroProductPageContent
                        ...
                        onDistributionModelTrendOpen={handleDistributionModelTrendOpen}
                      />
                    </div>
                  </div>
                </div>
        ...
        <RankingTrendPopover
          ...
          country={trendCountryValue}
          segment={deck?.metadata.selectedSegment ?? "SUV A0"}
          fuelTypes={selectedFuelTypes}
          onClose={() => setTrendDrawer({ open: false, brand: "", sourceTable: rankingTrendSourceTableForSalesMode(salesMode) })}
          onBack={trendDrawer.model ? () => setTrendDrawer((current) => ({ ...current, model: undefined })) : undefined}
          onModelClick={(model) => setTrendDrawer((current) => ({ ...current, model }))}
        />
      </div>
    </div>
  );
...
function HeroProductPageContent({ deck, pageKey, priceSource, priceEditor, salesMode, distributionLayout, specColumns, customColumns, selectedTrackingCountry, fuelLabel, segmentLabel, onDistributionCountryTrace, onDistributionModelTrendOpen }: { deck: HeroProductDeckResponse; pageKey: HeroProductPageKey; priceSource: HeroProductPriceSource; priceEditor: HeroProductPriceEditorBinding; salesMode: HeroProductSalesMode; distributionLayout: HeroProductDistributionLayout; specColumns: HeroProductSpecColumnKey[]; customColumns: HeroProductSpecColumnOption[]; selectedTrackingCountry: MarketScanCountryOption; fuelLabel: string; segmentLabel: string; onDistributionCountryTrace: (country: string) => void; onDistributionModelTrendOpen: (brand: string, model: string) => void }) {
  ...
  if (pageKey === "topDistribution") {
    return <DistributionSlide page={deck.pages.topDistribution} variant="top" salesMode={salesMode} layout={distributionLayout} selectedTrackingCountry={selectedTrackingCountry} onTrackCountry={onDistributionCountryTrace} onModelTrendOpen={onDistributionModelTrendOpen} />;
  }
  ...
  return <DistributionSlide page={deck.pages.heroDistribution} variant="hero" salesMode={salesMode} layout={distributionLayout} selectedTrackingCountry={selectedTrackingCountry} onTrackCountry={onDistributionCountryTrace} onModelTrendOpen={onDistributionModelTrendOpen} />;
}
    ...
    <div className="hero-product-page-stack hero-product-page-stack--benchmark">
      ...
      <div className="market-scan-grid market-scan-grid--two-wide hero-product-benchmark-grid">
        ...
        <HeroProductPanel
          ...
          title="续航、性能、技术、价格对标"
          subtitle={`中国车型下沉到核心规格，并用 ${priceSource.toUpperCase()} 价格源对齐。`}
...
function TrendSlide({ page, priceSource, priceEditor, variant, selectedTrackingCountry }: { page: HeroProductDeckResponse["pages"]["topTrend"]; priceSource: HeroProductPriceSource; priceEditor: HeroProductPriceEditorBinding; variant: "top" | "hero"; selectedTrackingCountry: MarketScanCountryOption }) {
  const insight = buildTrendInsight(page, priceSource, variant);
  const channelRows = page.models.slice(0, variant === "hero" ? 6 : page.models.length);
  const countryRanking = countryRankingForTrendPage(page, selectedTrackingCountry);
  const trackingCountryLabel = countryRanking.country.label || countryRanking.country.value;
      ...
      <div className="market-scan-grid hero-product-trend-grid">
        ...
        >
          ...
          <div className="hero-product-trend-visual-grid">
            <div className="hero-product-trend-chart">
              <div className="hero-product-trend-chart-head">
                <span>Volume Trend</span>
                <strong>All markets</strong>
              </div>
              <TrendSvg series={page.series} />
            </div>
            <CountryRankTrace ranking={countryRanking} />
          </div>
        </HeroProductPanel>
...
function alignedCountryOrder(items: HeroProductDeckResponse["pages"]["topDistribution"]["distribution"]["items"], fallbackCountries: string[]): string[] {
  ...
  const seen = new Set<string>();
  function pushCountry(country: string): void {
    const normalized = country.trim();
    if (!normalized || seen.has(normalized)) return;
    ordered.push(normalized);
    seen.add(normalized);
  }
  firstColumnCountries.forEach(pushCountry);
  fallbackCountries.forEach(pushCountry);
  for (const item of items) {
    item.countries.forEach((country) => pushCountry(country.country));
  }
  return ordered;
}
...
function DistributionSlide({ page, variant, salesMode, layout, selectedTrackingCountry, onTrackCountry, onModelTrendOpen }: { page: HeroProductDeckResponse["pages"]["topDistribution"]; variant: "top" | "hero"; salesMode: HeroProductSalesMode; layout: HeroProductDistributionLayout; selectedTrackingCountry: MarketScanCountryOption; onTrackCountry: (country: string) => void; onModelTrendOpen: (brand: string, model: string) => void }) {
  const [dragCountry, setDragCountry] = useState("");
  ...
  const visibleItems = page.distribution.items;
  ...
  const countryDensityClass = maxCountryRows > 34 ? " is-very-dense" : maxCountryRows > 24 ? " is-dense" : "";
  const columnDensityClass = visibleItems.length >= 28 ? " is-top-30" : visibleItems.length > 10 ? " is-top-20" : "";
  const matrixStyle = {
      ...
      if (state.current) {
        ...
        window.setTimeout(() => {
          if (dragStateRef.current.suppressClickCountry === state.current) {
            dragStateRef.current.suppressClickCountry = "";
          }
        }, 400);
  ...
  function beginCountryDrag(country: string): void {
  ...
  function commitCountryClick(country: string): void {
    if (dragStateRef.current.suppressClickCountry === country) {
      dragStateRef.current.suppressClickCountry = "";
      return;
    }
    onTrackCountry(country);
  }
  ...
  return (
    <div className="hero-product-page-stack hero-product-page-stack--distribution">
      <HeroProductInsightCallout insight={insight} />
      <HeroProductPanel
        eyebrow={variant === "hero" ? "Market · China Models" : "Market · Top Models"}
        title={page.title}
        subtitle={`${modeLabel}口径 · ${layoutLabel}；双击车型列头打开销量折线图；点击或拖动国家行更新追踪国家，当前 ${trackingCountryLabel}。${layout === "aligned" ? "按国家顺序统一行轴，缺失国家保留空行。" : "每个车型独立成列，按国家销量排序。"}`}
        className={`hero-product-distribution-panel hero-product-distribution-panel--${variant}${layout === "aligned" ? " is-country-aligned" : ""}${countryDensityClass}${columnDensityClass}`}
      >
        ...
        {visibleItems.map((item) => {
          ...
          const trendBrand = item.sourceBrand || item.brand;
          const trendModel = item.sourceModel || item.model;
          return (
            <section key={`${item.brand}-${item.model}`} className="hero-product-distribution-card">
              <header
                className="hero-product-distribution-card-head"
                title={`双击查看 ${item.model} 销量折线图`}
                role="button"
                tabIndex={0}
                onDoubleClick={() => onModelTrendOpen(trendBrand, trendModel)}
                onKeyDown={(event) => {
                  if (event.key === "Enter") {
                    event.preventDefault();
                    onModelTrendOpen(trendBrand, trendModel);
                  }
                }}
              >
                <strong title={item.model}>{item.model}</strong><span>总计 {formatNumber(item.totalSales)}</span>
              </header>
              <div className="hero-product-country-list">
                {countryRows.map((country) => (
                  <CountryDriveRow
                    key={country.country}
                    country={country}
                    maxSales={modelMaxCountrySales}
                    isTracking={countryMatchesCountry(country.country, activeCountryLabel)}
                    onPointerDown={beginCountryDrag}
                    onPointerEnter={previewCountryDrag}
                    onTrackCountry={commitCountryClick}
                  />
                ))}
              </div>
...
function countryMatchesCountry(country: string, selectedCountry: string): boolean {
  const normalizedCountry = normalizeCountryText(country);
  return Boolean(normalizedCountry && normalizedCountry === normalizeCountryText(selectedCountry));
}

function CountryDriveRow({ country, maxSales, isTracking, onPointerDown, onPointerEnter, onTrackCountry }: { country: HeroProductDistributionCountryRow; maxSales: number; isTracking: boolean; onPointerDown: (country: string) => void; onPointerEnter: (country: string) => void; onTrackCountry: (country: string) => void }) {
...
function ChannelMixStrip({ rows }: { rows: HeroProductModelRow[] }) {
  const densityClass = rows.length >= 28 ? " is-top-30" : rows.length >= 18 ? " is-top-20" : "";
  const stripStyle = {
    "--hero-product-channel-cols": String(Math.max(1, rows.length)),
  } as CSSProperties;
  return (
    <div className={`hero-product-channel-strip hero-product-channel-strip--pies${densityClass}`} style={stripStyle}>
