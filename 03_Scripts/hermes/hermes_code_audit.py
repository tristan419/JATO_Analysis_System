...
def check_api_frontend_consistency(files: list[str], full_diff: str) -> list[dict]:
    ...
    backend_routes_changed = any(
        "routes/" in f or "schemas.py" in f
        for f in files
        if f.startswith("06_AppPlatform/backend")
    )
    frontend_types_changed = any(
        FRONTEND_TYPES_FILE in f for f in files
    )
...
def check_db_schema_migration(files: list[str], full_diff: str) -> list[dict]:
    ...
    findings = []
    models_changed = any(
        "models.py" in f and f.startswith("06_AppPlatform/backend")
        for f in files
    )
    migration_added = any(
        "alembic" in f.lower() and "migration" in f.lower()
        for f in files
    )
...
def check_env_var_documentation(files: list[str], full_diff: str) -> list[dict]:
    ...
    if new_env_vars and EXAMPLE_ENV_FILE not in files:
        findings.append({
            ...
            "finding": f"New env var(s) referenced but {EXAMPLE_ENV_FILE} was not updated: {', '.join(sorted(new_env_vars)[:5])}",
            "suggestedFix": f"Add the new env vars to {EXAMPLE_ENV_FILE} with safe defaults (commented out if secret).",
        })
    return findings
...
def check_registry_updates(files: list[str], full_diff: str) -> list[dict]:
    ...
    scraper_changed = any(
        f.startswith("07_ScrapingToolkit/") for f in files
    )
    if scraper_changed:
        for reg_file, reg_name in REGISTRY_FILES.items():
            if reg_file not in files:
                findings.append({
                    ...
                    "suggestedFix": f"Review and update {reg_file} if new sources, pipelines, or artifacts were added.",
                })

    # Airflow DAG changes → Pipeline Registry
    airflow_changed = any(
        "airflow/dags/" in f for f in files
    )
    ...
    systemd_changed = any(
        "systemd" in f and (f.endswith(".service") or f.endswith(".timer"))
        for f in files
    )
    ...
    new_routes = any(
        f.startswith("06_AppPlatform/backend/app/api/routes/") and
        any(f.endswith(ext) for ext in [".py"])
        for f in files
    )
...
def check_duplicate_scheduling(files: list[str], full_diff: str) -> list[dict]:
    ...
    if has_new_schedule:
        findings.append({
            ...
            "suggestedFix": "Check hermes/pipeline_registry.yaml for duplicate scheduling. See gap.pipeline.duplicate_news_scheduling for an existing example.",
        })
...
def check_prompt_changes(files: list[str], full_diff: str) -> list[dict]:
    ...
    prompt_changed = any(
        any(pf in f for pf in prompt_files) and
        f.startswith(("06_AppPlatform/backend", "07_ScrapingToolkit"))
        for f in files
    )
    ...
    if prompt_changed:
        findings.append({
            ...
            "finding": "A file containing LLM prompts was modified. Prompt version should be bumped if content changed.",
...
def check_frontend_type_consistency(files: list[str], full_diff: str) -> list[dict]:
    ...
    if types_changed:
        findings.append({
            ...
            "finding": "frontend/src/types/index.ts was modified. Verify backend serializers produce matching shapes.",
...
def check_artifact_deletion(changes: list[dict]) -> list[dict]:
    ...
    critical_paths = [
        "03_Scripts/", "07_ScrapingToolkit/", "06_AppPlatform/",
        "airflow/", ".github/workflows/",
    ]
    critical_deletes = [
        d for d in deleted
        if any(d.startswith(p) for p in critical_paths)
    ]
    if critical_deletes:
        findings.append({
            "severity": "NEEDS_REVIEW",
...
def check_docs_updated(files: list[str]) -> list[dict]:
    ...
    has_code_changes = any(
        f.startswith(("06_AppPlatform/", "07_ScrapingToolkit/", "03_Scripts/", "airflow/"))
        for f in files
    )
    has_doc_changes = any(
        f.startswith("Markdown_Readme/") for f in files
    )
...
def _generate_report(results: dict) -> str:
    ...
    lines.append(f"- **Blockers:** {s['blockerCount']} | **Needs Review:** {s['needsReviewCount']} | **Warnings:** {s['warningCount']} | **Info:** {s['infoCount']}")
    lines.append("")
    ...
    for f in results["findings"]:
        sev = f"🔴 {f['severity']}" if f['severity'] == 'BLOCKER' else (
            f"🟠 {f['severity']}" if f['severity'] == 'NEEDS_REVIEW' else (
                f"🟡 {f['severity']}" if f['severity'] == 'WARNING' else f"🔵 {f['severity']}"
            )
        )
