    ...
    for f in sorted(base.glob("*.py")):
        ...
        dags.append({
            ...
            "schedule": schedule_match.group(1) if schedule_match else (
                cron_match.group(1) if cron_match else (
                    schedule_attr.group(1).strip() if schedule_attr else None
                )
            ),
        })
    return dags
...
def _discover_python_scripts() -> list[dict]:
    ...
    key_patterns = [
        "sync", "fetch", "enrich", "ingest", "dryrun", "batch",
        "refresh", "elt", "partition", "precompute", "crawl", "scrape",
        "run_", "_runner", "_fetcher", "_enricher",
    ]
...
def check_registry_coverage(
    ...
    for cat, items in discovered.items():
        for item in items:
            ...
            matched = any(
                item_path in rp or rp in item_path
...
def check_duplicate_scheduling(
    ...
    if news_gh:
        findings.append({
            "severity": "MEDIUM",
            "area": "Duplicate Scheduling",
            "finding": f"Country News Sync has GitHub Actions workflow: {[w['name'] for w in news_gh]}. Verify it is workflow_dispatch only (no schedule).",
        ...
        for prod_id in (producers or []):
            if not any(_safe_dict(p, "pipelineId") == prod_id for p in registry_pipelines):
                findings.append({
                    ...
                    "finding": f"Artifact '{art_name}' references producer '{prod_id}' not found in Pipeline Registry",
                    "artifactId": art_id,
                })
        ...
        for cons_id in (consumers or []):
            if not any(_safe_dict(f, "featureId") == cons_id for f in registry_features):
                findings.append({
                    ...
                    "finding": f"Artifact '{art_name}' references consumer '{cons_id}' not found in Feature Registry",
                    "artifactId": art_id,
                })
...
def check_source_failure_tracking(
    ...
    if not status_data:
        findings.append({
            ...
            "finding": "No scheduled_fetch_status.json available. Cannot verify per-source error tracking for VOC/News/MSRP.",
            "recommendation": "Implement per-source failed_sources.json output in voc_fetcher.py and news_runner.py.",
        })
        return findings
        ...
        if failed > 0:
            ...
            if not has_per_source:
                findings.append({
                    "severity": "WARNING",
                    "area": "Source Failure Tracking",
                    "finding": f"VOC reports {failed} failed sources but no per-source breakdown (sourceId, url, error type)",
                    "recommendation": "Add per-source error tracking to voc_fetcher.py. Output failed_sources.json with source code, error type, timestamp.",
                })
...
def score_pipeline_risk(
    ...
    pipeline_findings = [
        f for f in findings
        if str(pid) in str(f.get("finding", ""))
        or str(pid) in str(f.get("pipelines", ""))
    ]
    if any(f["severity"] in ("HIGH", "BLOCKER") for f in pipeline_findings):
        return "high"

    covered = any(
        str(pid) in str(c.get("pipeline", ""))
        for c in status_coverage.get("coveredPipelines", [])
    )
...
def _generate_report(results: dict) -> str:
    ...
    if sc.get("exists"):
        ...
        if covered:
            ...
            lines.append("|---|---|---|---|---|")
            for c in covered:
                lines.append(f"| {c['pipeline']} | {c['status']} | {c.get('lastRunAt', '?')} | {c.get('successCount', 0)} | {c.get('failedCount', 0)} |")
...
def run_audit(status_json_path: str | None = None) -> dict:
    ...
    return {
        ...
        "sourceFailureTracking": [
            f for f in all_findings if f["area"] == "Source Failure Tracking"
        ],
...
def main() -> None:
    ...
    print(f"  Registered: {s['registeredPipelines']} | Systemd: {s['discoveredSystemd']} | Airflow: {s['discoveredAirflow']}")
    print(f"  Duplicates: {s['duplicateSchedulingRisks']} | Missing reg: {s['missingRegistryEntries']} | High risk: {s['highRiskFindings']}")
