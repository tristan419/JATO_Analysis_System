...
BRAND_OVERRIDES = {
    ...
    "HYUNDAI": lambda cc, url: re.sub(r"hyundai\.\w+/", f"hyundai.{TLD_MAP.get(cc,cc)}/", url).replace(
        "hyundai.hu/", f"hyundai.{TLD_MAP.get(cc,cc)}/"
    ),
    ...
    "SKODA": lambda cc, url: re.sub(r"skoda(-auto)?\.\w+/", f"skoda.{TLD_MAP.get(cc,cc)}/", url).replace(
        "skoda-auto.", "skoda."
    ),
    ...
    "TESLA": lambda cc, url: re.sub(
        r"tesla\.com/\w+_\w+/", f"tesla.com/{cc.lower()}_{TLD_MAP.get(cc,cc).upper()}/", url
    ),
