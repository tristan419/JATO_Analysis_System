...
python3 - "$MIHOMO_CONTROLLER_URL" "$MIHOMO_PROXY_URL" "$MIHOMO_TEST_URL" "$MIHOMO_DELAY_TIMEOUT_MS" "$MIHOMO_MAX_CANDIDATES" <<'PY'
import json
import subprocess
import sys
import time
...
controller_url, proxy_url, test_url, timeout_ms, max_candidates_text = sys.argv[1:]
...
ranked.sort()
if not ranked:
    print("[mihomo-select] WARN: no candidate passed controller delay test", file=sys.stderr)
    raise SystemExit(0)

for _, proxy_name in ranked[:20]:
