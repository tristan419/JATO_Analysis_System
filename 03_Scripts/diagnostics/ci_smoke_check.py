...
def run_regression_checks() -> None:
    run([PYTHON_BIN, "03_Scripts/diagnostics/regression_csv_download_guardrails.py"])
    run([PYTHON_BIN, "03_Scripts/diagnostics/regression_render_strategy_defaults.py"])
    run([PYTHON_BIN, "03_Scripts/diagnostics/regression_time_selector_consistency.py"])

    if has_dashboard_dataset():
        run([PYTHON_BIN, "03_Scripts/diagnostics/regression_filter_option_pushdown.py"])
    else:
        print(
...
def main() -> None:
    ...
    run([PYTHON_BIN, "03_Scripts/diagnostics/style_check.py"])
    run([PYTHON_BIN, "-m", "py_compile", *files])

    run([PYTHON_BIN, "03_Scripts/elt_worker.py", "--help"])
    run([PYTHON_BIN, "03_Scripts/data_pipeline/prepare_monthly_raw_update.py", "--help"])
    run([PYTHON_BIN, "03_Scripts/raw_compare_review.py", "--help"])
    run([PYTHON_BIN, "03_Scripts/data_pipeline/build_partitioned_dataset.py", "--help"])
    run([PYTHON_BIN, "03_Scripts/data_pipeline/run_data_refresh_job.py", "--help"])
