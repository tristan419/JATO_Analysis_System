...
>
> 状态：`PLANNED_BLOCKED_BY_PR_215`
>
> 规划基线：`main@619466e81528045f59ea64ad9bcdf69c60a219f8`
>
> 规划分支：`codex/bom-colour-rules-unified`
>
...
- PR #215：`OPEN`、`Draft`、`BEHIND`，head `a4dc1381f9182194f6ec03817b0914dfd5145e8b`。
- #215 相对本规划基线为 2 commits ahead、18 commits behind，merge-base 为 `37a9905cebd0ccaf5147314f6cedb639f95a25ca`。
- #215 修改 7 个文件。
...
1. 在 #215 合并或 owner 明确协调前，不编辑 TypeScript/Python 业务代码。
2. 首选顺序是 #215 先更新到最新 main、通过并合并。
3. 随后颜色实现必须从“届时最新 remote main”重新建立干净 worktree/branch；本规划文档可以迁移，但不得把旧 main 树带入实现 PR。
4. 不把 #215 历史 branch merge 进本分支，不从旧 `bom-colour-rule-library` 恢复代码。
5. PR description 必须列出最终共享文件、owner 和与 #215 的继承关系。
...
预计 7–9 个代码/测试文件，净改动约 350–550 行，不含测试夹具。若实现开始引入新数据表、第二套规则 service 或专项迁移，则视为范围失控，必须停止。
...
- [x] 写入本目标、范围、交互和验收契约。
- [ ] #215 同步最新 main、通过并合并。
- [ ] 从届时最新 main 重建颜色实现 worktree/branch。
