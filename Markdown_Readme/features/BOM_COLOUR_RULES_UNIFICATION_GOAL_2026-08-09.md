...
已在独立分支 `codex/bom-colour-reprice-audit` 提交实现 `95706e19`，并以 `f72ce28b` 补齐测试契约，修复新增颜色及其他写入口把 Dual/Special 当成无加价 Single 的旁路。统一 repository 决策现在负责：effective tier（显式 `colour_tier`，再回退合法 legacy type）、同模板同国家 Single 基准、同档位定制/品牌默认 surcharge，以及缺档位/缺规则的可识别跳过原因。
