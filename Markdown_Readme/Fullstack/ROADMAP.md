...
- 进行中：增量入库后续能力与运维标准化；Dashboard 窄屏导航重构、collapsed rail 语义修正、`filters/options` 延迟治理第一阶段已落地，第二阶段缓存与预计算增强待补。
...
- 移动端顶部导航已从 card nav 补齐为 hamburger/drawer 结构。
- `全维度筛选` 桌面折叠态已改为 toggle-only rail，不再保留竖排文字。
- `filters/options` 第一阶段已完成接口降载、请求 abort 和前端 options cache；PostgreSQL 仍不作为首轮性能修复手段。
