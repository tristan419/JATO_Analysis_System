...
本节覆盖下方“C/D 已完成、只需合并”的旧结论。本轮只更新方案文档，未改业务代码或部署。

- 新增双色不加价不限于 J5：现有流程先在无 FOB 时重算，再复制来源最终价并落为 manual_edit，后续加价又被手动保护跳过。C@579ea01e 尚未修复这条通用路径。
...
- 特殊价表、API 和后端匹配已存在；在现有 Colour Surcharges 中接入列表与编辑，Special 色卡提供同一入口。优先级为车型色码特例→品牌色码特例→品牌 Special 默认；0 表示免加价，停用才回退。补实际规则来源展示、价格预览/应用结果、停用，以及新增 SKU 的事务内正确初始化。详见 [BOM 实施文档 C5–C8](../features/BOM_ADMIN_CANDIDATE_ACCEPTANCE_2026-09-15.md)。
