...
- [ ] **2026-09-25 共享颜色映射实现待 Candidate**：新 worktree `/Users/litristan/Downloads/JATO_Analysis_System_bom_shared_colour_mapping`、分支 `codex/bom-shared-colour-mapping`，基线 `cf10438fdeed`。本地已让 Add/Edit Colour 复用现有品牌＋色码规则；色码缺失时按颜色名/别名找唯一候选，多个候选必须用户选择，未知值不猜 HEX、不持久化默认灰色。BOM/Matrix 继续共用 `colourHex`；双色两份 HEX、Matte 一份 HEX，颜色数量不推导 tier。后端聚焦 19 passed/36 deselected，前端 75 files/411 tests、类型检查、构建、路由回归均通过；尚未创建 PR、合入 main 或准备 Candidate。下一步只走本业务 PR→最新 main→一次 Candidate，再按 BOM 颜色验收清单记录实际构件；不更新 Active/www/intl。
