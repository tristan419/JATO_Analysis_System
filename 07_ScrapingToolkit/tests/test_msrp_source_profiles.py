...
def test_czech_volkswagen_tayron_profile_uses_current_official_list_price_pdf() -> None:
    source_path = (
        Path(__file__).resolve().parents[1]
        / "source_drafts"
        / "suv_only_country_model_top30"
        / "cz"
        / "24_volkswagen_tayron_cz.yaml"
    )
    source = _load_yaml_mapping(source_path)
    profile = _build_pdf_text_profile(source["profile"])
    ...
    assert source["source_url"] == profile.url
    assert profile.prefer_curl_download is True
    assert profile.default_price_label == "Cena s DPH"
    assert profile.match_reason["document_valid_from"] == "2026-04-01"
    assert len(profile.entry_patterns) == 15
    for entry in profile.entry_patterns:
        assert "Cena bez DPH" not in entry.pattern
        assert re.compile(entry.pattern)
...
def test_belgium_audi_q6_e_tron_profile_uses_only_the_official_including_vat_price() -> None:
    source_path = (
        Path(__file__).resolve().parents[1]
        / "source_drafts"
        / "suv_only_country_model_top30"
        / "be"
        / "05_audi_q6_e_tron_be.yaml"
    )
    source = _load_yaml_mapping(source_path)
    profile = _build_scrapling_profile(source["profile"])
    entry = profile.text_regex.entry_patterns[0] if profile.text_regex else None
    ...
    assert profile.text_regex.include_element_html is True
    assert entry is not None
    assert "Q6\\s+SUV\\s+e-tron" in entry.pattern
...
def test_belgium_volvo_ex30_profile_excludes_finance_and_monthly_payments() -> None:
    source_path = (
        Path(__file__).resolve().parents[1]
        / "source_drafts"
        / "suv_only_country_model_top30"
        / "be"
        / "20_volvo_ex30_be.yaml"
    )
    source = _load_yaml_mapping(source_path)
    profile = _build_scrapling_profile(source["profile"])
    entry = profile.text_regex.entry_patterns[0] if profile.text_regex else None
    ...
    assert source["extractor_type"] == "scrapling"
    assert source["source_type"] == "manufacturer_official"
    assert source["source_url"] == "https://www.volvocars.com/nl-be/cars/ex30-electric/"
    assert profile.url == source["source_url"]
    assert profile.tier == "http"
    assert profile.network_idle is False
    assert profile.load_dom is False
    ...
    assert "EX30\\s*<span[^>]*>Essential" in entry.pattern
...
def test_belgium_volvo_ex40_profile_excludes_finance_and_business_edition_offers() -> None:
    source_path = (
        Path(__file__).resolve().parents[1]
        / "source_drafts"
        / "suv_only_country_model_top30"
        / "be"
        / "29_volvo_ex40_be.yaml"
    )
    source = _load_yaml_mapping(source_path)
    profile = _build_scrapling_profile(source["profile"])
    entry = profile.text_regex.entry_patterns[0] if profile.text_regex else None
    ...
    assert source["extractor_type"] == "scrapling"
    assert source["source_type"] == "manufacturer_official"
    assert source["source_url"] == "https://www.volvocars.com/nl-be/cars/ex40-electric/"
    assert profile.url == source["source_url"]
    assert profile.tier == "http"
    assert profile.network_idle is False
    assert profile.load_dom is False
    ...
    assert entry is not None
    assert "equipmentLevelsItem:title" in entry.pattern
    assert "EX40\\s*<span[^>]*>Essential" in entry.pattern
    assert "equipmentLevelsItem:price" in entry.pattern
    assert "incl\\.\\s*btw" in entry.pattern
    assert "Financi" not in entry.pattern
    assert "Lease" not in entry.pattern
    assert "Business Edition" not in entry.pattern
    assert "maand" not in entry.pattern
    assert "(?P<price>\\d{2}\\.\\d{3})" in entry.pattern
    assert entry.price_label == "Vanaf incl. btw"
    assert re.compile(entry.pattern)
